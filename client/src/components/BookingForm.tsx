...
export default function BookingForm({ isOpen, onClose, selectedDate, editBooking, existingBookings, initialPeriod }: BookingFormProps) {
  ...
  const createMutation = useMutation({
    mutationFn: async (values: FormValues) => {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: 'فشل في إنشاء الحجز' }));
        throw new Error(errorData.message || 'فشل في إنشاء الحجز');
      }
  ...
  const updateMutation = useMutation({
    mutationFn: async (values: FormValues & { id: number }) => {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: 'فشل في تحديث الحجز' }));
